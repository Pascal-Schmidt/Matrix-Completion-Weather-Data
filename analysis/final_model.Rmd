...
train %>%
  dplyr::mutate_at(vars(WINDDIR, WINDSPEED, TEMPERATURE, DEWPOINT, PRESSURE),
                   .funs = ~imputeTS::na_interpolation(., maxgap = 5)) -> train
```
...
test %>%
  ...
  dplyr::mutate(column = dplyr::case_when(col == 1 ~ "WINDDIR",
                                          col == 2 ~ "WINDSPEED",
                                          col == 3 ~ "TEMPERATURE",
                                          col == 4 ~ "DEWPOINT",
                                          col == 5 ~ "PRESSURE")) -> to_be_predicted
...
for(i in 1:nrow(df)) {
  ...
  if(train %>%
    ...
    lm(formula, 
       data = train %>%
         dplyr::filter(USAF == train$USAF[[df$row[[i]]]])) -> lm_model
...
for(i in 1:nrow(temp_dew_df)) {
  ...
  if(temp_dew_df$column[[i]] == "TEMPERATURE") {
    ...
    lm(TEMPERATURE ~ DEWPOINT + as.factor(MONTH),
       data = train %>%
         dplyr::filter(USAF == train[temp_dew_df$row[[i]], ]$USAF)) -> lm_model
...
train %>%
  dplyr::mutate_at(vars(WINDDIR, WINDSPEED, TEMPERATURE, DEWPOINT, PRESSURE),
                   .funs = ~imputeTS::na_interpolation(.)) -> train
